...
//
// TestCorpusBehavior_CSharp is LANG-03's D-12 acceptance gate for C# — now
...
//
// The old env-var-based resolver (CODEGRAPH_CSHARP_CORPUS) and its sibling-
// checkout fallback with t.Skip are retired — this test always runs against
// the Phase-1-locked serilog corpus when that corpus is present, and fails
...
// TestCorpusBehavior_CSharp validates, against the Phase-1-locked serilog corpus:
//
...
//     counts.
func TestCorpusBehavior_CSharp(t *testing.T) {
	corpusDir := lockedCorpusDir(t, "csharp")
	...
	if stats1.Files == 0 {
		t.Fatalf("indexer.Run found 0 files in the locked C# corpus %s", corpusDir)
	}
	...
	// absence of that construct in whichever corpus is configured.
