...
- [x] `internal/upgrade/brew_test.go` — table-driven detection tests for UPGR-02, including the
      ...
      — **verified 2026-08-11:** rows `caskroom-no-receipt-not-detected` (`:203`) and
      `cellar-no-receipt-not-detected` (`:210`) both execute
- [x] Constructed-tree fixtures under `t.TempDir()` for `/opt/homebrew`, `/usr/local`, a custom
      prefix, and linuxbrew — **both Caskroom and Cellar shapes at the linuxbrew prefix**
      (D-04R, 2026-08-11)
      — **verified 2026-08-11:** `brew_test.go:169-198` builds 4 prefixes
      (`apple-silicon` → `opt/homebrew`, `intel` → `usr/local`, `custom` →
      `opt/custom-brew-prefix`, `linuxbrew` → `home/linuxbrew/.linuxbrew`) × both tree shapes,
      unconditionally — linuxbrew is not special-cased, so D-04R holds by construction
- [x] Dangling-symlink and symlink-loop fixture modes, pinning the single `EvalSymlinks` error
      ...
      — **verified 2026-08-11:** `buildDanglingSymlinkFixture` (`:101`),
      `buildSymlinkLoopFixture` (`:134`); floors at `:274` (`len(rows) < 16`) and `:284`
      (not-detected `< 7`); measured live run yields exactly 16 subtests
- [x] Seam assertion in `internal/upgrade/upgrade_test.go` proving `Options.download` and
      `Options.swap` are never invoked under a detected brew install (D-08)
      — **verified 2026-08-11:** `TestUpgradeRun_RefusesBrewManagedCask`,
      `TestUpgradeRun_CheckBrewManagedStepsAside`,
      `TestUpgradeRun_ForceDoesNotOverrideBrewRefusal` all PASS
...
| Resolved | 0 |
| Escalated | 0 |
| Automated commands executed verbatim | 15 / 15 |
| Passing | 15 |
| Failing | 0 |

**Method.** Every one of the 15 `<automated>` commands in the Per-Task Verification Map was
executed verbatim (markdown `\|` escapes stripped to `|` per the table-escaping convention),
not read. This follows the Phase-3 finding recorded in `4686200t19`: reading a declared command
cannot detect a phantom target or a pattern that matches nothing, and `go test -run PATTERN`
exits 0 when the pattern matches nothing. Wave 0's four requirements were each confirmed
against the shipped test source with file and line citations, not from the plan text.

**Measurements worth recording.** Three floors are satisfied *exactly*, not with slack —
`TestDetectBrewManaged` yields exactly 16 subtests against a `< 16` guard, its not-detected
count is exactly 7 against a `< 7` guard, and `rg -c '^\s*cosign verify-blob' Taskfile.yml`
returns exactly 5 against a `-ge 5` floor. Exact-satisfaction is the intended state here (the
floors were calibrated to measured post-task counts rather than padded), but it means any
future removal goes RED immediately — which is the point — and equally that any addition
silently raises the slack. Recalibrate the floor when the count legitimately changes rather
than letting drift accumulate.

**Why `nyquist_compliant: false`.** One proof is one-time by nature, exactly the sense recorded
for Phase 3 in `4686200t19` — it does NOT mean anything is untested. Task `04-06-01`'s
artifact-reading legs (`RUN_ID` binding, `TRAP_ARMED=1`, the payload-hash cardinality and
distinctness legs, `RESTORE_VERDICT`/`RESTORE_INVOCATIONS`) read three files under `$TMPDIR`
that only a fresh real `brew install` acceptance run can regenerate. Re-executed successfully
on 2026-08-11 while those artifacts survived (`run-20260811T175959Z-35769`; `tap no → untapped`;
`trust yes → left-trusted`), but after a reboot or a tmp sweep the command fails closed with
`missing or empty acceptance artifact` rather than passing vacuously. Its live-machine legs
(`brew list --cask`, `brew tap`, `brew trust --json v1`, the man-directory probe) remain
re-runnable indefinitely, so this is a weaker `false` than Phase 3's — one artifact-dependent
proof, not four unautomated requirements, and sampling continuity is not violated (all 15 tasks
carry an `<automated>` verify).

**Follow-up recommended (not a gap).** If durable re-sampling of `04-06-01` is wanted, persist
the three acceptance artifacts into the phase directory alongside `04-EVIDENCE.md` instead of
leaving them in `$TMPDIR`. That would make the gate re-runnable without a second real install,
at the cost of committing run transcripts. Deliberately not done here — it is a decision, not a
defect, and `04-EVIDENCE.md` already records the receipt values verbatim in-repo.
