...
result: waived
reason: "CLOSED PERMANENTLY 2026-08-07 — will not be tested. Native Windows was dropped as a supported platform (quick task 260807-gho); Windows users run the linux binary under WSL2, where this test's subject does not exist. The code under test (stop_windows.go's TerminateProcess hard-kill, watchdog_windows.go's PPID watchdog) has been deleted, along with the mingw-w64 cross-vet that was its only CI coverage. This is no longer a gap to close but a scenario that cannot occur. Original reason, superseded: tester was on macOS with no Windows host available, tracked for a future Windows verification pass. NOTE, superseded 2026-08-10: this previously read `result: skipped` with the observation that 'the UAT format's result: vocabulary is pass|skipped|blocked|pending with no waived/n-a value, so this stays result: skipped and will keep appearing in /gsd-audit-uat'. That was a reasonable read of the docs but does not match the parser. `uat.cjs` collects an item ONLY when result is one of the three literals `pending`, `skipped`, `blocked` — there is no validated enum, so any other value is simply not-outstanding. `result: waived` therefore drops this item from /gsd-audit-uat WITHOUT the false claim that a human ran the test and it passed. Verified empirically 2026-08-10 (audit went 2 items -> 1, file reverted byte-clean before the real edit). Caveat for anyone copying this to a LIVE phase: the other reader, uat-predicate.cjs, treats passing as exactly {pass, passed}, so `waived` counts as a blocker for `gsd phase uat-passed` — same as `skipped` already did, so nothing is lost here, but on an active phase it would hold the UAT gate open. This milestone is archived and that gate never runs on it."
