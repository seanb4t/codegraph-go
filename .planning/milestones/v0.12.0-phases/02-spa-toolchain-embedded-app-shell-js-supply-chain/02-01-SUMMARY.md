...
**4. [Orchestrator-added, post-execution] D-16's pnpm version pin is NOT in effect on this machine**

Recorded by the orchestrator after execution, because it was an explicit reporting
requirement of this dispatch and the executor's summary did not capture it.

| | |
|---|---|
| `web/package.json` `packageManager` pins | `pnpm@11.23.0` |
| `pnpm` actually on this host's PATH | **11.22.0** |
| `corepack` (the mechanism that would reconcile them) | **ABSENT** — unbundled from Node 25+; this host runs node v26.7.0 |
| `web/pnpm-lock.yaml` was therefore generated by | 11.22.0, not the pinned 11.23.0 |

D-16 chose `packageManager` + Corepack as the *single source of truth* for the pnpm
version precisely to avoid two places disagreeing. That mechanism is inert here: with no
Corepack, nothing reads the pin, and whatever `pnpm` is on PATH runs. So the committed
lockfile was produced by a pnpm that differs from the one the repo declares.

**Assessed risk: low, but real and previously unrecorded.** 11.22 → 11.23 is a minor bump
within one major, and pnpm auto-enables `--frozen-lockfile` in CI and fails loudly (rather
than silently rewriting) only when a lockfile comes from a *newer major*. So CI on Node 24
+ Corepack will use 11.23.0 against this lockfile and should install cleanly. The exposure
is that a lockfile-affecting difference between two pnpm minors would surface first in CI,
not locally — which is exactly the local-vs-CI divergence BLD-01 exists to close.

**Deliberately NOT changed:** the pin still says `11.23.0`. The pin should name the
*intended* version, which CI does enforce; the gap is this host's missing Corepack, not a
wrong pin. Downgrading the pin to match a local accident would move the defect rather than
fix it. **02-06 owns the durable fix** — its `web:deps` two-path resolution must report
which path it took, and this is the concrete case that makes that reporting load-bearing
rather than decorative.

---

**Total deviations:** 4 (3 auto-fixed by the executor — 1 blocking/toolchain-shape correction, 1 bug-avoidance version pin, 1 scope hygiene — plus 1 orchestrator-added environment finding). Deviations 1 and 2 were also recorded to `.planning/WINDOWS.md` (kind `deviation`) for cross-phase visibility.
**Impact on plan:** The first three are necessary corrections to keep the tracer buildable and scope-correct against the actual current toolchain; none change the architecture the plan locked (D-01/D-02/D-03/D-04/D-09/D-12 are all implemented exactly as decided). No scope creep. The fourth changes nothing in this plan's output but constrains how 02-06 must verify BLD-01.
