...
- **D-01:** **`zig cc` cross-compilation from a single Linux runner via GoReleaser**, `CGO_ENABLED=1`, targeting all 6 platforms (darwin/linux/windows × amd64/arm64). This matches the documented approach in `CLAUDE.md`/`STACK.md`/`PARSER-DECISION.md` (`goreleaser/example-zig-cgo`: set `CC`/`CXX` to `zig cc -target <triple>`). A single builder also keeps the SLSA provenance and reproducibility stories coherent.
  - **Documented fallback (researcher must validate before committing):** if macOS (needs the macOS SDK for framework linking) or Windows CGo linking via `zig cc` proves untenable, fall back to a **native-runner matrix** (ubuntu/macos/windows GitHub runners each build their own platform). PARSER-DECISION.md explicitly deferred this cross-compile validation to Phase 8 and it was **never actually executed in the spike** (zig not installed in that environment) — so validating a real 6-target CGo build is the first, highest-risk task of this phase.

### Build → sign → provenance → SBOM wiring (DIST-02, DIST-03)
- **D-02:** **GoReleaser builds; cosign signs; the SLSA *generic* generator attests; syft SBOMs; govulncheck gates.**
  - GoReleaser v2 builds all 6 targets (with D-01's `zig cc`), produces the checksums file, and emits the SBOM via its `sbom:` (syft) block.
  - **cosign v3 keyless** signs the checksums file using GitHub Actions OIDC (`id-token: write`) — no long-lived key.
  - **SLSA3 provenance via `generator_generic_slsa3.yml` (the generic generator) over GoReleaser's artifacts/checksums — NOT `builder_go_slsa3.yml`.** Rationale: the Go SLSA builder rebuilds the binary under its own fixed config and does not support a `zig cc` CGo cross-build; running provenance over the already-built artifacts avoids that conflict while still producing verifiable SLSA3 provenance.
  - `govulncheck ./...` (call-graph-aware) runs as a **blocking CI gate**; dependency scanning (govulncheck is the primary; a generic SCA on the SBOM is acceptable as secondary) also gates.
