...
| 04-02-02 | 02 | 1 | UPGR-02 | T-04-07 | Removing a CI gate cannot silently remove an unrelated one; the rehearsal independently re-checks freshness; the shape tests are proved to have RUN via named PASS counts, never a package-level `ok` match | config (shape + named-PASS counts) | `test "$(rg -c -e 'SENTINEL' Taskfile.yml \|\| echo 0)" -eq 0 && test "$(rg -c -e 'FRESH_MAN_PAGE_COUNT' Taskfile.yml)" -ge 3 && test "$(go test ./internal/upgrade/ -run '^TestTaskfile' -v 2>&1 \| rg -c -e '^--- PASS: TestTaskfile')" -ge 5 && test "$(go test ./internal/upgrade/ -run '^TestWorkflowRunBodiesInvokeTask$' -v 2>&1 \| rg -c -e '^--- PASS: TestWorkflowRunBodiesInvokeTask')" -eq 1` | ✅ | ⬜ pending |
...
| 04-05-03 | 05 | 3 | UPGR-01 | T-04-16, T-04-18 | The drift guard is observed RED three times: a real semantic loosening, an empty match set, and the dropped-Task-1 regression the previous floor could not detect | unit (RED proof ×3 + revert) | `go test ./internal/upgrade/... && git diff --exit-code Taskfile.yml internal/upgrade/taskfile_shape_test.go` | ❌ W0 | ⬜ pending |
...
| 04-06-02 | 06 | 4 | UPGR-01, UPGR-03 | T-04-19 | An EXIT trap installed before the first mutating byte restores the payload on every path, proved POSITIVELY by a receipt with equal before/after hashes — not by an absence check that passes when `brew` is missing | acceptance (observed exit codes + positive restoration receipt) | `R="${TMPDIR:-/tmp}/codegraph-04-06-restore-receipt.txt"; command -v brew >/dev/null \|\| exit 1; test -f "$R" \|\| exit 1; test "$(rg -c -e '^RESTORE_VERDICT=ok$' "$R")" -eq 1 && test "$(rg -o -e '^PAYLOAD_SHA256_(BEFORE\|AFTER)=[0-9a-f]{64}$' "$R" \| rg -o -e '[0-9a-f]{64}$' \| sort -u \| wc -l \| tr -d ' ')" -eq 1 && test "$(brew list --cask --versions codegraph \| wc -l \| tr -d ' ')" -eq 0` | ✅ | ⬜ pending |
| 04-06-03 | 06 | 4 | UPGR-01, UPGR-02, UPGR-03 | T-04-20, T-04-22 | The unproved leg is named, not claimed; every carried assumption is dispositioned, with the ID↔disposition association enforced per ID rather than by a line count | docs (structure assertion, one independent check per token) | `E=.planning/phases/04-codegraph-upgrade-homebrew/04-EVIDENCE.md; test -s "$E" && test "$(rg -c -e 'UPGR-ACCEPTANCE-EVIDENCE' "$E")" -eq 1 && for r in UPGR-01 UPGR-02 UPGR-03; do test "$(rg -c -e "^\\\| $r \\\|" "$E")" -eq 1 \|\| exit 1; done` | ❌ W0 | ⬜ pending |
...
|----------|-------------|------------|-------------------|
| The refusal is observed against a **real** `brew tap seanb4t/tap` + `brew install codegraph` from the Phase-3 tap | UPGR-01 | The ROADMAP's Depends-on clause states a synthetic layout can only prove the mechanism, not that it matches what actually ships. Requires a real published cask and a real Homebrew install on the host. | **Blocking precondition (review cycle 1, HIGH): the run HALTS if a codegraph cask is already installed** — an arbitrary prior cask version is not reliably restorable from a tap that publishes only its current cask, matching the rehearsal's own refusal at `Taskfile.yml:1671`. Then `brew trust seanb4t/tap` (per `khb8v67c48` — the published contract does not work without it), `brew install codegraph`, then `codegraph upgrade` and `codegraph upgrade --check`; record both outputs and exit codes verbatim. All mutating steps run inside one script with `trap … EXIT INT TERM` installed before the first mutating byte, emitting a restore receipt that the automated gate asserts positively. |
