...
| 04-06-02 | 06 | 4 | UPGR-01, UPGR-03 | T-04-19 | An idempotent EXIT trap installed before the first mutating byte restores the payload on every path, proved POSITIVELY by a receipt with equal before/after hashes, `RESTORE_INVOCATIONS=1`, and a `TAP_ACTION` that agrees with Task 1's recorded `TAP_PREEXISTING` — never by an absence check that passes when `brew` is missing, and never by untapping unconditionally | acceptance (observed exit codes + positive restoration receipt + bidirectional baseline match) | `R="${TMPDIR:-/tmp}/codegraph-04-06-restore-receipt.txt"; B="${TMPDIR:-/tmp}/codegraph-04-06-baseline.env"; command -v brew >/dev/null \|\| exit 1; test -f "$R" \|\| exit 1; test -f "$B" \|\| exit 1; test "$(rg -c -e '^RESTORE_VERDICT=ok$' "$R")" -eq 1 && test "$(rg -c -e '^RESTORE_INVOCATIONS=1$' "$R")" -eq 1 && test "$(rg -o -e '^PAYLOAD_SHA256_(BEFORE\|AFTER)=[0-9a-f]{64}$' "$R" \| rg -o -e '[0-9a-f]{64}$' \| sort -u \| wc -l \| tr -d ' ')" -eq 1 && test "$(brew list --cask --versions codegraph \| wc -l \| tr -d ' ')" -eq 0 && B="$B" R="$R" node -e 'const fs=require("fs");const pre=(/^TAP_PREEXISTING=(yes\|no)$/m.exec(fs.readFileSync(process.env.B,"utf8"))\|\|[])[1];const act=(/^TAP_ACTION=(untapped\|left-in-place)$/m.exec(fs.readFileSync(process.env.R,"utf8"))\|\|[])[1];if(!pre)throw new Error("baseline carries no TAP_PREEXISTING line");if(!act)throw new Error("receipt carries no TAP_ACTION line");const want=pre==="yes"?"left-in-place":"untapped";if(act!==want)throw new Error("tap restoration wrong: TAP_PREEXISTING="+pre+" requires TAP_ACTION="+want+", receipt says "+act);console.log("tap restoration OK: "+pre+" -> "+act)' && test "$(brew tap \| rg -c -e '^seanb4t/tap$' \|\| echo 0)" -eq "$(rg -c -e '^TAP_PREEXISTING=yes$' "$B" \|\| echo 0)" && test "$(ls "$(brew --prefix)/share/man/man1/" \| rg -c -e '^codegraph' \|\| echo 0)" -eq 0` | ✅ | ⬜ pending |
| 04-06-03 | 06 | 4 | UPGR-01, UPGR-02, UPGR-03 | T-04-20, T-04-22 | The unproved leg is named, not claimed; every carried assumption is dispositioned, with the ID↔disposition association enforced per ID rather than by a line count, and each Leg token asserted independently rather than by a combined threshold | docs (structure assertion, one independent check per token) | `E=.planning/phases/04-codegraph-upgrade-homebrew/04-EVIDENCE.md; test -s "$E" && test "$(rg -c -e 'UPGR-ACCEPTANCE-EVIDENCE' "$E")" -eq 1 && for t in 'Leg 1' 'Leg 2' 'Leg 3'; do test "$(rg -c -F -e "$t" "$E")" -ge 1 \|\| { echo "missing heading token: $t" >&2; exit 1; }; done && for r in UPGR-01 UPGR-02 UPGR-03; do test "$(rg -c -F -e "$r" "$E")" -ge 1 \|\| { echo "missing requirement ID: $r" >&2; exit 1; }; done && for r in UPGR-01 UPGR-02 UPGR-03; do test "$(rg -c -e "^\\\| $r \\\|" "$E")" -eq 1 \|\| { echo "no disposition-table row for $r" >&2; exit 1; }; done` | ❌ W0 | ⬜ pending |
...
|----------|-------------|------------|-------------------|
| The refusal is observed against a **real** `brew tap seanb4t/tap` + `brew install codegraph` from the Phase-3 tap | UPGR-01 | The ROADMAP's Depends-on clause states a synthetic layout can only prove the mechanism, not that it matches what actually ships. Requires a real published cask and a real Homebrew install on the host. | **Two blocking preconditions, both HALT the run.** (1) Review cycle 1, HIGH: HALT if a codegraph cask is already installed — an arbitrary prior cask version is not reliably restorable from a tap that publishes only its current cask, matching the rehearsal's own refusal at `Taskfile.yml:1671`. (2) Review cycle 2, HIGH: HALT if any `codegraph*.1` man page already exists under `$(brew --prefix)/share/man/man1/` — the cask's uninstall hook globs that SHARED directory unconditionally (`.goreleaser.yaml:621`) and would delete pages the run never created. Then `brew trust seanb4t/tap` (per `khb8v67c48` — the published contract does not work without it), `brew install codegraph`, then `codegraph upgrade` and `codegraph upgrade --check`; record both outputs and exit codes verbatim. All mutating steps run inside one script whose EXIT trap is installed before the first mutating byte, with INT/TERM legs that disarm the traps, run the idempotent cleanup once and exit. **The tap is untapped only if the run added it** (review cycle 2, HIGH), decided from the `TAP_PREEXISTING` value the baseline probe recorded BEFORE tapping. The trap emits a restore receipt that the automated gate asserts positively. |
