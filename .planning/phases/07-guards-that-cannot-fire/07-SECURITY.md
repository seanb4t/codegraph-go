...
Audit notes (2026-09-09): register origin is plan-time (`<threat_model>` present in all four PLAN files); no SUMMARY carried a `## Threat Flags` section. The ASVS-L1 short-circuit (no auditor subagent) was taken deliberately, not inherited: the register has no critical rows and no irreversible publish, every mitigate row above cites a file and line read directly at HEAD, and `07-REVIEW.md` (deep) had already reproduced all four guard families independently and RED-demonstrated the two gaps it found (CR-01, WR-01), both of which are now closed with transcripts in `07-MUTATION-LOG.md`. Phases 2 and 4 refused this short-circuit because those conditions did not hold there. Verification at HEAD `0d5d188a`: `go test ./internal/bench/ ./internal/query/archtest/ ./internal/upgrade/` green; `shellcheck scripts/inject-cosign-key.sh` clean; `.goreleaser.yaml`, `post-release-verify.yml`, `go.mod`, `go.sum` unchanged. Out of scope for this register: the sibling finding that `internal/graphstore/archtest` ignores per-package load errors is tracked as a pending todo (commit 1879aea1), not a Phase 07 threat.
