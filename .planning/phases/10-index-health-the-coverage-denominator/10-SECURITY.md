...
| T-10-16 | Tampering (post-review, 2026-09-13) | a live store holder (UI/MCP per-request open, in-flight Sync flush, or a concurrent index/sync) surviving `codegraph index`'s pre-wipe `priorCoverageGeneration` read past `graphstore.Open`'s ~400 ms retry budget makes the floor read `0`, so the rebuild stamps `CoverageGeneration = 1` and a page token minted before the wipe can validate against the new graph (the CR-01 shape, narrowed to a lock-collision window) | medium | accept | Deterministic shape CLOSED by `83666cce` (`TestIndexForceRebuildBumpsCoverageGeneration` — two real `codegraph index --force` runs, pre-wipe token → `query.ErrAborted`); the residual lock-collision window is below the `high` block threshold, requires an active holder at the exact rebuild instant, and is tracked as WINDOWS.md #36 with the fix named (distinguish `ErrNotFound` from `ErrStoreLocked`/other in `index.go:30-48`, hard-error before `RemoveAll`, hold-the-lock regression test per `open_lock_test.go`) — 10-REVIEW.md WR-01 (iteration 4) | closed |
...
| R-10-05 | T-10-16 | Post-review residual: `priorCoverageGeneration`'s failure-to-0 fallback cannot distinguish "never indexed" from "locked/corrupt at this instant"; the window needs a live holder to outlast the open retry during a `--force` rebuild. Accepted below the `high` gate with the fix specified in WINDOWS.md #36 rather than silently carried. The user authorized exactly one extra fix pass beyond the review-loop cap; that pass closed the deterministic critical (CR-01), and this narrower residual is recorded, not fixed, in this phase | orchestrator (autonomous run) | 2026-09-13 |
...
| 2026-09-13 | 17 | 17 | 0 | orchestrator reconciliation after the deep code review (10-REVIEW.md iterations 1–4): CR-01 cursor-resume-by-value → fixed `725ba1fb`; WR-01 wall-clock generation aliasing → fixed `fbdf17f3`/`10e9c028` (`Meta.coverage_generation = 10`); CR-01′ wipe-resets-generation → fixed `83666cce`; T-10-16 residual lock-collision window → accepted, WINDOWS.md #36 |
