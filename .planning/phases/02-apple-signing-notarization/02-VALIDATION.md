...
| 02-06 T3 | 02-06 | 4 | SIGN-02 | T-02-18, T-02-19 | cosign-verifies-before-chmod; suite job aborts on a bad download and fails if zero tests ran (counted from `-json` events) | unit (taskfile shape) + lint | `go test ./internal/upgrade/ -run 'TestVerifyNotarizedSuiteDeclaresNamedPreconditions\|TestVerifyGatekeeperDeclaresNamedPreconditions\|TestRehearseNotarizeDeclaresCredentialPreconditions' -v && task lint:actionlint` | ❌ test created by this task | ⬜ pending |
