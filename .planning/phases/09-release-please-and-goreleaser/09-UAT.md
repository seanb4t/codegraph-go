...
scope_correction: "PASS STANDS, but this test's title is broader than what it verified. What was tested and demonstrated is the VERIFICATION INSTRUCTION — fixed command PASSES, pre-fix command FAILS. The DESCRIPTIVE claim that generated the wrong instruction is still present in three places (found by 09-REVIEW.md WR-02, independently confirmed): release.yml:327 'over the assemble job's checksums file', docs/RELEASE.md:26-28 'provenance ... generated over the checksums file', docs/RELEASE-PROCEDURES.md:120-122 'runs SLSA3 build provenance ... over the checksums file'. release.yml:190-197's own correction block names that exact phrasing as the source that propagated into both docs — so the root cause survives inside the file documenting its own fix. No runtime impact: the SLSA bundle's subjects genuinely ARE the six binaries (parsed and verified this session). Risk is that a reader re-derives the broken verification command from the uncorrected prose. Tracked as WR-02, not reopened as a UAT gap, because what test 40 asserted was verified true."
result: pass
