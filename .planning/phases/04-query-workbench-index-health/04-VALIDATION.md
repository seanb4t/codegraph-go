...
| 04-05-T1 | 04-05 | 2 | HLT-01, HLT-02 | T-04-17 | no second verdict function; blank-roots mismatch is FALSE; two-snapshot disagreement has a stated display rule | unit | `cd web && pnpm exec vitest run … tests/health-view.test.ts` + count floor 8 (`&&`-chained) | ❌ new | ⬜ pending |
...
| 04-05-T3 | 04-05 | 2 | HLT-01, HLT-02, HLT-03 | T-04-17, T-04-21 | verdict precedes numbers (DOM order); warning present AND absent both asserted; stale-snapshot notice asserted | component | `cd web && pnpm exec vitest run … tests/health-page.test.ts` + count floor 9 (`&&`-chained) | ❌ new | ⬜ pending |
