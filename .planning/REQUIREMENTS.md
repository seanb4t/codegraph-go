...
- [ ] **FIX-02**: Every UI route serves a codegraph favicon that loads under the unchanged `default-src 'self'` CSP — shipped as a static file, not a `data:` URI and not a widened `img-src` (WINDOWS #30)
...
- [ ] **NUDGE-04**: The nudge fires at most once per session via a session-scoped sentinel and is silent with zero overhead in a repo without `.codegraph/` (NUDGE-02's property preserved)
...
- [ ] **CODEX-05**: A Codex PreToolUse nudge in `hooks.json` carries the same additionalContext-only, once-per-session contract as NUDGE-03/04; it is installed only when Codex's hooks feature is enabled (opt-in), skipped with a message otherwise, and its experimental status is stated in the docs
