...
- **Every guard in this milestone must carry a positive assertion that it did its work** (rule `84d1gfpywd`). A negative-only guard passes vacuously, and this repo has now diagnosed that shape five separate times — in v0.12.0 Phase 5 the same threat survived three green guards. Each of `GRD-01`…`GRD-05` reports a count, or a matched-anchor confirmation, of what it actually inspected, and each is demonstrated RED against a confirmed-applied, byte-cleanly-reverted mutation before it is trusted green. `GRD-06` is that proof committed rather than asserted — five demonstrations with pasted failing output, following `03-MUTATION-LOG.md`.
...
**Requirements**: GRD-01, GRD-02, GRD-03, GRD-04, GRD-05, GRD-06
**Success Criteria** (what must be TRUE):
  ...
  4. A test parses `post-release-verify.yml`, reports how many jobs it inspected, and fails when the event-aware conclusion guard is removed or inverted on any one of them; separately, the tap App secret-distinctness test reads both secret names from the real release-please and tap workflow files and fails when either workflow changes its name to collide — neither test can pass by comparing constants it wrote itself (GRD-04, GRD-05)
  5. A committed mutation log carries, for each of the five guards, the pasted failing output from its RED demonstration plus evidence of a byte-clean revert, following `03-MUTATION-LOG.md`'s precedent — five demonstrations, none of them summarised away (GRD-06)

**Notes**: Every fix has an exact source location and an exact reusable structural precedent already in-tree — `internal/graphstore/archtest/import_graph_test.go` for `GRD-02`, `internal/upgrade/bench_workflow_shape_test.go` for `GRD-04`/`GRD-05` — so this phase should skip the research pass. `GRD-01`'s fix must not itself be vacuous: a bare positivity floor is trivially satisfiable by a different measurement bug, so the test exercises the historical degenerate frame rather than a synthetic one. A project-wide mutation-testing framework is out of scope by construction; the hand-authored RED-demonstration convention already works at this scale. Consumes backlog **999.4**, three pending todos (dry-run-signed, post-release-verify, tap secret) and the T-01-18 archtest item. `GRD-07`/`GRD-08` were declined for this milestone by the maintainer and are recorded in REQUIREMENTS.md → v2, not silently dropped. One open scoping question carried in: whether the archtest should also forbid a `query`→`indexer` edge, which directly changes where Phase 10's helper may live — resolve it here, not there.
**Plans**: TBD
...
|-------|----------------|--------|-----------|
| 7. Guards That Cannot Fire | 0/TBD | Not started | - |
