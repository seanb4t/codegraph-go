...
- [x] **Phase 10: Local Build Tooling & CONTRIBUTING** - `Taskfile.yml` wrapping the build/test/lint/release-check workflows + contributor-facing CGo toolchain docs (promoted from backlog 999.1 on 2026-07-27) (completed 2026-08-03)
...
- [x] 10-04-PLAN.md — Runner identity in the perf baseline, bench.yml onto Namespace, and a reviewed Namespace baseline committed
- [x] 10-05-PLAN.md — `check:cross` replaces release-please.yml's inline pre-tag sweep, guarded against .goreleaser.yaml drift

**Wave 4** *(blocked on Wave 3 completion)*

- [x] 10-06-PLAN.md — Runner-aware regression refusal; perf and reproducibility gates onto Namespace via task targets
