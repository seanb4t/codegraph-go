...
**Notes**: Consumes seed **SEED-002**. `homebrew_casks:` is the block, not the deprecated `brews:` (GoReleaser v2.10; Homebrew's own maintainers recommend casks for GoReleaser-precompiled binaries per Homebrew/brew #20291). The default `GITHUB_TOKEN` cannot write cross-repo, so a dedicated least-privilege PAT is required. `gh`/`lazygit`'s build-time updater-disable ldflag does not transfer — casks ship precompiled binaries — so the cask-compatible analogue is a `homebrew_casks.hooks.post.install` sentinel. **Amended 2026-08-11 (D-02, plan 04-03):** this clause previously closed by naming that sentinel as the strongest available signal for Phase 4's detection to rely on. D-02 falsified that: Phase 4 removed the sentinel outright and keys on structural detection instead — the resolved install path (`filepath.EvalSymlinks`) plus Homebrew's own `INSTALL_RECEIPT.json` install receipt, a signal Homebrew itself authors rather than one this project's own release pipeline writes and can omit.
...
**Goal**: Neither install path lies about what is installed — `codegraph upgrade` recognizes a Homebrew-managed install, steps aside with an actionable pointer, and never mutates the Caskroom or Cellar behind brew's back.
**Depends on**: Phase 3 for **acceptance**. Implementation can proceed in parallel with Phases 2–3 against a constructed Caskroom-and-Cellar-shaped symlink tree in a temp dir, and the `Run`-branch logic is unit-testable through the existing `internal/upgrade` func-var seam — but the phase is not done until it has been run against the real tap Phase 3 publishes, because a synthetic layout can only prove the mechanism, not that it matches what actually ships.
**Requirements**: UPGR-01, UPGR-02, UPGR-03
**Success Criteria** (what must be TRUE):

  1. **Amended 2026-08-11 (D-01, D-08, plan 04-03):** After a genuine `brew tap` + `brew install codegraph` from the Phase-3 tap, `codegraph upgrade` refuses, names `brew upgrade codegraph` in its message, and neither `Options.download` nor `Options.swap` is ever invoked — asserted directly through the injectable func-var seam already in `upgrade.go:38-43`. Previously this criterion proved the refusal via a sha256/mtime comparison on the Cellar binary; a hash comparison is an indirect proxy for "did our code call `swap`" and drags Homebrew's filesystem and rollback semantics into a test of our own branch. The real-tap acceptance run is retained but now only has to observe the refusal, on either the Caskroom or Cellar tree (UPGR-01)
  2. **Amended 2026-08-11 (D-09, D-10, plan 04-03):** `codegraph upgrade --check` under that same install steps aside with the same resolved-path pointer rather than resolving a version — `brew outdated codegraph` already owns that answer for brew installs, and a second source risks disagreeing with it — and mutates nothing, asserted via the same seam (neither `Options.download` nor `Options.swap` invoked) plus an exit-0 verdict; `brew upgrade codegraph` still succeeds afterward. This criterion previously promised the available version number; it is amended to match REQUIREMENTS.md UPGR-03's own wording, "reports how to upgrade" — a criterion amendment, not a requirement change (UPGR-03)
...
- [x] 04-01-PLAN.md — tracer: `internal/upgrade/brew.go` resolves symlinks, matches the Caskroom/Cellar shape, requires Homebrew's own install receipt, and `Run()` refuses before any network call — proved by the seam assertion, plus the full prefix/tree table and the `--check` step-aside
- [x] 04-02-PLAN.md — remove the Phase-3 sentinel from both cask hooks and every rehearsal assertion (D-02), close UF-5 with an mtime-baseline man-page assertion, and correct `03-EVIDENCE.md`'s false stranding claim
- [x] 04-03-PLAN.md — amend ROADMAP criteria 1–3, the Phase-4 checkbox item, the milestone-ordering bullet, Phase 3's Notes, REQUIREMENTS UPGR-01/02 and PROJECT.md (D-01, D-04R, D-08, D-09)

**Wave 2** *(blocked on Wave 1 completion)*

- [x] 04-04-PLAN.md — `--help`, `README.md` and `docs/RELEASE.md` state the refusal, the pointer command and both exit behaviours (D-10), replacing three published claims this phase falsifies
...
- [x] 04-06-PLAN.md — acceptance against the real Phase-3 tap: detection observed firing on a genuine Homebrew-authored tree, both exit behaviours observed, machine restored, and the one unproved leg named rather than claimed
