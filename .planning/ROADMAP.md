...
- [x] 05-01-PLAN.md — Tracer: `codegraph://tools/explore` registered, listed, read and wire-observed end-to-end; 25 frozen transcripts re-captured under one named cause

**Wave 2** *(blocked on Wave 1 completion)*

- [x] 05-02-PLAN.md — The remaining 9 resource files: 7 per-tool fact-sheets plus the `CODEGRAPH_MCP_TOOLS` filter and index-state behavior docs
...
- [x] 05-04-PLAN.md — Wire fan-out: a read scenario per advertised URI, the unindexed catalog, the unknown-URI error shape, and the oracle re-proved non-vacuous
...
- [x] 06-01-PLAN.md — SessionStart nudge package: script, `.claude/settings.json` registration, Phase-7 embed fragment, executed-in-both-trees behavioural test (NUDGE-01, NUDGE-02)
- [x] 06-02-PLAN.md — SKILL.md structural contract as a guard test, then the skill skeleton: frontmatter, decision table, skip condition, resource-URI pointers (SKILL-01)
...
- [x] 06-04-PLAN.md — live-session rehearsal and committed verification artifacts (SKILL-03, plus the live half of NUDGE-01/02)
...
- [x] 07-01-PLAN.md — Tracer: one `install` writes the embedded SKILL.md, executable nudge script, and SessionStart registration into Claude Code's real read locations, idempotently (AGENT-01)

**Wave 2** *(blocked on Wave 1 completion)*

- [x] 07-02-PLAN.md — Uninstall removes exactly what install wrote, plus the {install, uninstall} × {read-error, malformed} matrix and one shared read posture for `.claude/settings.json` (AGENT-02)
...
- [x] 07-04-PLAN.md — `codegraph upgrade` re-invokes `Install()` for every previously-configured location after a successful swap; refresh failure warns rather than failing the upgrade (AGENT-03)
...
| 7. `codegraph install` Skill + Hooks Distribution | 4/4 | In Progress|  |
