...
**Versioning note:** "v1.0" is a *planning-milestone* name, never a release version. The shipped artifact line is `v0.2.0`, computed by release-please from Conventional Commits; there is deliberately no `v1.0.0` tag (maintainer directive D-06R, 2026-07-29). Milestone tags carry a `milestone-` prefix (`milestone-v0.1`, `milestone-v1.0`) specifically so they fall outside `release.yml`'s `push: tags: "v[0-9]*"` trigger.
