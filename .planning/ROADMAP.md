...
- **The sweep removes framing, never capability.** `codegraph migrate` keeps working end-to-end against a legacy `.codegraph/` SQLite index and is reframed as a legacy-index import (`CODE-03`). `internal/indexer/tsextract`, the language registry and the capability matrix are product surface — TypeScript-the-indexed-language, not TypeScript-the-origin-project. Every retained use is resolved term-by-term with a recorded reason, never by regex.
...
**Goal**: A contributor filing an issue or opening a PR, and an agent reading the source, meet a project described on its own terms — with TypeScript-the-indexed-language and `codegraph migrate` both fully intact.
...
**Success Criteria** (what must be TRUE):
  ...
  5. `codegraph migrate` still imports a legacy `.codegraph/` SQLite index end-to-end against a real fixture, and its `--help` text, docs and comments describe it as a legacy-index import rather than a port or parity feature (CODE-03)
