...
- [x] 04-07-PLAN.md — `web:components:drift`, render-cost measurement, committed-bundle refresh
...
- [x] 05-07-PLAN.md — In-place expansion of a file into its symbols, and the final rebuilt bundle
...
- [x] 06-05-PLAN.md — Graph stability: no-layout fast path, authoritative survivor position write-back, layout generation token against overlapping runs, and the guava-scale displacement measurement
- [x] 06-06-PLAN.md — The browser gate ordinary tests cannot satisfy: three real tabs with per-tab timing paired to a received-generation superset, fan-out isolation with the blocked tab proven blocked, and a jittered reconnect behind a fixed-port proxy that survives a server restart
- [x] 06-07-PLAN.md — Criterion 5's real-process gate: real `daemon` + `serve --mcp` + `ui` against one store, flush duration bounded against a measured no-UI baseline; plus the recorded lifecycle verdict and the phase's single bundle rebuild
- [x] 06-08-PLAN.md — Criterion 1's gap closure, found by live UAT after verification passed: the root Status route — the default landing view — subscribes to the live store, re-issuing the `GetStatus` it already owns
