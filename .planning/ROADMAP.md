...
- [x] **Phase 8: Release Hardening & Benchmarks** - Signed, attested, reproducible static-binary releases with published head-to-head benchmarks and CI regression gates (completed 2026-07-13)
...
- [x] 06-05-PLAN.md — internal/version package + `version`/`telemetry` commands + help ergonomics (CLI-01/03)

**Wave 2** *(blocked on Wave 1)*

- [x] 06-02-PLAN.md — JSON agent targets: Claude/Cursor/Gemini/Kiro/Antigravity (+ --path, no-type, project-root, self-heal quirks) (AGNT-01/02/03)
- [x] 06-03-PLAN.md — Format agent targets: Codex TOML / opencode JSONC (hujson) / Hermes YAML surgery (AGNT-01/02/03)
- [x] 06-06-PLAN.md — `upgrade [version] [--check]`: redirect-trick release resolution + sigstore-go verify-before-swap + atomic self-replace + sigstore-go pin (CLI-02)
...
- [x] 07-02-PLAN.md — graphstore additive PutMigration/GetMigration progress-record interface (MIGR-02)
...
- [x] 07-04-PLAN.md — progress.go (resumable cursor) + swap.go (crash-safe atomic dir swap, D-07) (MIGR-02)

**Wave 3** *(blocked on Wave 2)*

- [x] 07-05-PLAN.md — validate.go: count reconciliation (edge-dedup aware) + dangling check (file: exempt) + drop-dangling (D-09) (MIGR-02)

**Wave 4** *(blocked on Wave 3)*

- [x] 07-06-PLAN.md — migrate.Run orchestration: detect→write(x/ index)→resume→validate→atomic swap + healthy gating (MIGR-01/02)
...
- [x] 08-01-PLAN.md — GoReleaser 6-target CGo build config + reproducibility flags + version ldflags + local build proof (DIST-01/DIST-04)
- [x] 08-02-PLAN.md — internal/bench OS-level peak-RSS capture + KB/bytes normalization + Metrics (TDD) (PERF-01/INDX-06)
- [x] 08-03-PLAN.md — tools/bench/gencorpus deterministic synthetic 100k+ corpus generator (TDD) (PERF-02/INDX-06)

**Wave 2** *(blocked on Wave 1)*

- [x] 08-04-PLAN.md — release.yml native-matrix build + per-binary cosign + SLSA + SBOM + publish + first-release checkpoint (DIST-01/02/03)
- [x] 08-05-PLAN.md — verify_release e2e test + releaseAssetName↔name_template finalization (TDD) (DIST-02)
- [x] 08-06-PLAN.md — internal/bench tolerance-band + absolute-ceiling regression gate (TDD) (PERF-02/INDX-06)

**Wave 3** *(blocked on Wave 2)*

- [x] 08-07-PLAN.md — tools/bench/runner (head-to-head + regression) + pinned real-corpus manifest + committed baseline.json (PERF-01/02/INDX-06)
...
- [x] 08-09-PLAN.md — docs/RELEASE.md (verify + DIST-05 audit narrative) + docs/BENCHMARKS.md (methodology + raw numbers) (DIST-05/02/03/PERF-01)
...
| 8. Release Hardening & Benchmarks | 9/9 | Complete   | 2026-07-13 |
