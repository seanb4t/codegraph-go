...
- [ ] 03-01-PLAN.md — Tracer: Modern `server/discover` end-to-end — `cacheScope: "private"` on discover, one sessionless discover-then-call scenario frozen, hand-authored cache-control anchor
...
- [ ] 03-04-PLAN.md — SPEC-05's live tool catalog: per-request index re-check driving `AddTool`/`RemoveTools`, proven on the wire by a real mid-session `codegraph init`

**Wave 4** *(blocked on Wave 3 completion)*

- [ ] 03-05-PLAN.md — `instructions` on both the Modern and Legacy path, the oldest era's tool call, and the phase's one reviewed re-freeze pass
...
| 3. `2026-07-28` Spec Compliance | v0.3.0 | 0/5 | Not started | - |
