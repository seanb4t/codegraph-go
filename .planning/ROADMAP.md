...
- **The docs tail lands last (Phase 12) because it documents the final flag surface,** including `BRW-12`'s new `codegraph ui --editor-url`. `DOCS-06`'s guard walks the *live* Cobra tree — hidden, inherited persistent and deprecated flags included — so the one blind spot of the `cobra/doc`-generated reference (hidden flags are omitted by design) is closed by an allowlist-with-reasons rather than by a hand-maintained document (amended 2026-09-13 at Phase 12 discuss).
...
- [x] 07-04-PLAN.md — GRD-04 per-job conclusion-guard test (removed and inverted both RED-proven), the tautological tap test deleted per D-09, family (d) and the log closed
...
- [x] 08-01-PLAN.md — TRACER: the `test/tmux` spine end to end (build tag, own `TestMain` resolver, tmux argv wrappers, the bounded stability poll), the TTY-02/TTY-03 empty-registry escape-hygiene assertion, TTY-01's named skip contract, and the `test:tmux` target with its executed-count equality gate

**Wave 2** *(blocked on Wave 1 completion)*

- [x] 08-02-PLAN.md — TTY-04 alt-screen entry/exit over a really-seeded daemon record, TTY-05 checkbox glyphs plus a whole-tree hash proving cancel writes nothing, TTY-06 idle frame stability with N reported

**Wave 3** *(blocked on Wave 2 completion)*

- [x] 08-03-PLAN.md — TTY-07's `tmux-e2e` CI job on `ubuntu-latest` with both run bodies bare task calls, the same-commit `inScopeJobs` entry, and the deliberate `tmux -V` bootstrap
...
**Wave 5** *(gap closure — G-08-1, blocked on Wave 4 completion)*

- [x] 08-05-PLAN.md — close G-08-1: `pollUntilStable` gains a required readiness predicate (convergence = ready AND byte-stable) so TTY-03 cannot converge on the pre-output frame of a cold first exec; deterministic poll self-test watched RED then GREEN, `TMUX_EXPECTED_TESTS` 6, `capture.go`'s false margin claim replaced with the cold-path measurements
...
- [x] 09-01-PLAN.md — TRACER: `GetEditorLink` as UIService's 15th rpc (closed enums, presets on the wire) proven through the real listener with confinement and the positive scheme allowlist; `--editor-url` / `--no-editor-url` / `CODEGRAPH_EDITOR_URL` / `CODEGRAPH_NO_EDITOR_URL` resolved at startup, malformed values refuse to bind
...
- [x] 09-03-PLAN.md — the ONE per-line `SourcePane` restructure with an always-rendered gutter, the sticky innermost-symbol breadcrumb with its honest empty state, the live Chromium gate `breadcrumb-check.mjs` against this repo's index, and `09-MUTATION-LOG.md` family (a) watched RED against the pre-fix binary

**Wave 3** *(blocked on Wave 2 completion)*

- [x] 09-04-PLAN.md — header "Open in editor" link from the load-time probe, one-rpc-per-click gutter targets, the per-browser localStorage override and the three-preset picker with provenance display; live gate extended with the editor-link phases
...
- [x] 10-04-PLAN.md — `/health` Coverage section: counts line, reason groups expanding to rows, extraction failures visibly distinct, first-class "Coverage unknown — re-index to record it", text-only remedy; vitest-level verification (Open Question 3: no Playwright gate); SPA rebuilt drift-clean
...
- [x] 10-06-PLAN.md — `10-MUTATION-LOG.md` families (a) D-15 unset-as-zero, (b) D-16 decoy rename, (c) D-14 present-tense reconstruction; `10-SECURITY.md` with every threat row test-or-verdict; validation map filled; phase-close gate
...
**Success Criteria** (what must be TRUE):
  ...
  1. `docs/CLI-REFERENCE.md` — generated from the live Cobra tree by `cobra/doc` via a committed `task docs:cli` target, with command prose carried in each command's `Long` — documents every non-hidden command and flag in the tree, including this milestone's new `--editor-url`, replacing what `docs/FLAG-PARITY.md` used to carry before it was deleted at v0.11.0; a `task docs:cli:drift` regenerate-and-diff gate (the `proto:drift` shape) fails when the committed file is stale (DOCS-05)
  2. A guard test walks the live Cobra tree including hidden, inherited persistent and deprecated flags, reports the number of commands and flags it inspected, and fails when any registered flag is unaccounted for — a non-hidden flag must appear in the generated reference; a hidden or deprecated flag must appear in a committed allowlist that names its reason — demonstrated RED by registering a throwaway *hidden* flag (which the generated reference does not carry), then byte-cleanly reverting it (DOCS-06)
  3. The brew-trust instructions in `README.md` and `docs/RELEASE.md` recommend the narrow `brew trust --cask seanb4t/tap/codegraph` grant with one sentence of security framing rather than the broader `--tap` grant; a documentation wording change verified by reading the diff — no guard and no test, per the Notes above (DOCS-07)
