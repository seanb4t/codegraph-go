...
Phase: 09 (release-please-and-goreleaser) — EXECUTING 09-07
Plan: 6 of 8 complete (09-01..09-06); executing 09-07, then 09-08
Status: CI GREEN on main@3ffdb5c — all three original blockers closed. govulncheck FIXED (grpc v1.82.1); internal/daemon FIXED (a real single-writer-invariant bug, not a flake) and verified on linux/amd64; perf gate FIXED — and the "REAL ~10.6% regression" previously recorded here was itself WRONG. There was never a code regression: baseline.json had been recorded on darwin/arm64 while the gate runs ubuntu-latest, and CheckRegression never compared GOOS/GOARCH, so the gate reported the distance between two machines. Same-platform control measured +0.73%. Fixed by a platform guard, a dispatchable ubuntu-latest rebless job, median-of-N, and a runner-matched baseline (d4672cf). Bisect todo resolved REFUTED — do not run it. The "fix CI first" precondition on 09-07 is therefore SATISFIED. Also fixed before dispatch (ba5a548): 09-07/09-08 Task 1 carried gate="blocking" while workflow.auto_advance=true, so auto-mode would have auto-selected 09-08's FIRST option — publish a permanent signed release — with no human input; both are now gate="blocking-human".
Last activity: 2026-07-31 — Perf blocker closed and CI green on main; gates hardened to blocking-human; began executing 09-07 (disposable live release proof) with the maintainer present
...
- [Phase ?]: selectByIndices is duplicated (not imported) into internal/cli/tui — internal/cli imports internal/cli/tui, so the reverse import would be a cycle; kept byte-identical dedup+ascending-order semantics
- [Phase ?]: uninstall's off-TTY/-y non-interactive default stays 'all' (its historical no-flag behavior), not 'auto' like install — only the interactive path is new for uninstall
...
| 260731-0a0 | Bump google.golang.org/grpc to v1.82.1, closing govulncheck GO-2026-6061 | 2026-07-31 | a0cfceb | [260731-0a0-fix-govulncheck-go-2026-6061-bump-google](./quick/260731-0a0-fix-govulncheck-go-2026-6061-bump-google/) |
