...
- **NUDGE-01 resume-matcher does not observably fire (Phase 6 live rehearsal, 2026-08-12).** `.claude/settings.json` registers the SessionStart `resume` matcher correctly (matches documented `--resume`/`--continue`/`/resume` syntax), but resuming a real session via `claude --resume <session-id>` produced zero `SessionStart:resume` hook events in that session's own transcript — confirmed by direct transcript inspection, not self-report. Only the original `startup` event fired. The `startup` matcher was independently confirmed working in the same rehearsal. See `.claude/skills/codegraph/verification/NUDGE-live-session.md` for the full capture. Not fixed in Phase 6 — needs its own debug session to determine whether this is a Claude Code runtime limitation or a project-side gap.
- **Newly-added project skill not surfaced to a fresh session (Phase 6 live rehearsal, 2026-08-12).** `.claude/skills/codegraph/SKILL.md` is correctly placed and committed, but a genuinely fresh Claude Code session's own skill-listing system reminder did not name `codegraph` at all — confirmed by grepping that session's raw transcript. The where-is-X routing test still passed on its literal criterion, but only because two other independent mechanisms (the operator's global `~/.claude/CLAUDE.md` CodeGraph section and the codegraph MCP server's own `instructions` string) already cover the same behavior — so this rehearsal could not cleanly attribute the pass to the new skill. See `.claude/skills/codegraph/verification/SKILL-03-rehearsal.md`'s Verdict section. Needs investigation into why project-skill discovery didn't pick up a newly-added, correctly-placed `.claude/skills/*/SKILL.md` in a fresh session.
