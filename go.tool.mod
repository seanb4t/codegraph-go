...
// go.tool-lint.mod) because adding a root go.mod `tool` directive for
// go-task/task was measured live to add 237 net-new modules to the main
// module's build list (511 -> 748, +46%), dominated by cloud.google.com/go/*
// transitive dependencies dragged in by goreleaser's cloud-storage backends.
// Those would enter this module's govulncheck (blocking, DIST-03) scope and
...
// YAML API loses that version bid to whatever task/goreleaser drag in
// transitively (verified: action_metadata.go:273:22:
// te.Errors[0].Error undefined (type string has no field or method Error)).
// See go.tool-lint.mod's own header for the full measurement.
//
