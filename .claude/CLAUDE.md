...
A pre-indexed code knowledge graph for coding agents (Claude Code, Cursor, Codex, Gemini, etc.) — one static Go binary with a CLI, MCP server, and auto-sync, no bundled runtime to install or manage, built for performance, a verifiable supply chain, and an architecture designed to grow into team-scale usage.
...
- **Compatibility**: **Retired as a constraint (v0.11.0, 2026-08-13).** codegraph-go's behavior is now defined by its own requirements and its own frozen goldens, not by another implementation's. This supersedes the 2026-08-09 "functionality baseline" ruling, which itself had already downgraded it from binding. Nothing of that shape binds any more: the one-way legacy-index migration command was removed entirely in Phase 5 (CODE-03, maintainer ruling D-04, 2026-08-15), so there is no read contract left to honor. See Key Decisions
...
- **Licensing**: MIT. Attribution for the project this one began as a reimplementation of lives in `NOTICE` — the single sanctioned place — and is discharged there, not restated across the repo. `LICENSE` stays verbatim MIT text (an appended attribution paragraph downgrades GitHub's license detection to `NOASSERTION`; observed, not assumed)
...
| `github.com/spf13/cobra` | v1.9.x | CLI framework | De facto standard for Go CLIs (kubectl, docker, gh, hugo); built-in shell-completion and man-page generation map directly onto the CLI's nested command surface (`init`, `install`, `uninstall`, `uninit`, `upgrade`, `explore`). |
