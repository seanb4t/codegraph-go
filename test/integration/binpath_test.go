...
// TestResolveTestBinPath is a table test over resolveTestBinPath's six
// documented behaviors (main_test.go's doc comment on the function). Every
// case also re-checks the "no third outcome" invariant inline: useEnv=true
// never pairs with a non-nil error, and a non-empty raw value never
// resolves to useEnv=false with a nil error — that absence is the property
// that forbids a silent fallback to a local build on a bad override.
