...
// go.tool-lint.mod (actionlint) and go.tool-proto.mod (buf, protoc-gen-go,
// protoc-gen-connect-go) for the same reason those three are isolated from
// each other and from the root go.mod: MVS resolves one version per module
// across the whole graph, and go.tool-lint.mod's own header already records
// a LIVE-VERIFIED collision from co-locating a new tool with an existing
// one — actionlint's expected YAML API lost its version bid to whatever
// task/goreleaser drag in transitively (action_metadata.go:273:22:
// te.Errors[0].Error undefined). golangci-lint vendors a substantial
// fraction of the Go static-analysis ecosystem (it wraps dozens of
// third-party linters, each with its own dependency tree) and is plausibly
// the largest single third-party executable tree this repository carries —
// the most likely candidate in this repository to win or lose an MVS
// version bid against something else. Rather than re-run the
// co-location-then-discover-the-collision experiment go.tool-lint.mod's
// header already documents, this modfile follows that established,
// already-proven isolation pattern from the outset.
//
...
// .planning/phases/03-browse-inspect-navigation/03-10-PLAN.md Task 1.
module github.com/seanb4t/codegraph-go/tools-golangci
